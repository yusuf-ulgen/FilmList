...
class AddContentViewModel(
    ...
    fun saveMediaContent(title: String, type: String, rating: Int, comment: String?, listId: Long, posterPath: String? = null, startDate: String? = null, endDate: String? = null) {
        viewModelScope.launch {
            val userId = sessionManager.userId.first()
            if (userId != null && userId != -1L) {
                ...
                val finalStartDate = startDate ?: SimpleDateFormat("dd.MM.yyyy", Locale.getDefault()).format(Date())
                val mediaContent = MediaContent(
                    userId = userId,
                    listId = listId,
                    title = title,
